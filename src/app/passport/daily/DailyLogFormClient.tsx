...
export default function DailyLogFormClient({ unitId, token }: { unitId: number; token: string }) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.inspectorName) return alert("Mohon isi nama pemeriksa");
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center p-12">
      ...
      <p className="text-xs font-black text-slate-400 uppercase tracking-widest">Checking today's log status...</p>
    </div>
  );
  ...
  if (existingLog) return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <div className="bg-emerald-50 border border-emerald-100 rounded-3xl p-6 flex flex-col items-center text-center">
        ...
        <h3 className="text-xl font-black text-emerald-900 tracking-tight">Data Sudah Terdaftar</h3>
        <p className="text-sm font-medium text-emerald-700/70 mt-1">
          Logsheet operasional untuk hari ini ({new Date().toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}) sudah diinput oleh <span className="font-black underline">{existingLog.inspector_name}</span>.
        </p>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-3xl p-6 space-y-6">
         <div className="flex items-center gap-3 pb-4 border-b border-slate-100">
            ...
            <h4 className="text-sm font-black text-slate-800 uppercase tracking-widest">Today's Reading Summary</h4>
         </div>
         
         <div className="grid grid-cols-2 gap-4">
            <SummaryStat label="Fan Status" value={existingLog.fan_on ? "ON" : "OFF"} />
            <SummaryStat label="Valve Opening" value={`${existingLog.valve_opening}%`} />
            <SummaryStat label="Supply Temp" value={`${existingLog.supply_temp}°C`} />
            <SummaryStat label="Room Temp" value={`${existingLog.room_temp}°C`} />
            <SummaryStat label="Pre-Filter" value={existingLog.filter_pre} highlight={existingLog.filter_pre === 'Kotor'} />
            <SummaryStat label="HEPA Filter" value={existingLog.filter_hepa} highlight={existingLog.filter_hepa === 'Kotor'} />
         </div>

         <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 italic text-xs text-slate-500">
           Sesuau protokol efisiensi, input logsheet dibatasi 1x per hari per unit untuk menjamin konsistensi data monitoring performa unit.
         </div>
      </div>
    </motion.div>
  );
  ...
  return (
    <div className="space-y-8 pb-20">
      <div className="flex flex-col gap-1">
        <h2 className="text-2xl font-black text-[#003366] tracking-tight">Daily Check Logsheet</h2>
        <p className="text-xs font-bold text-slate-400 uppercase tracking-[0.2em]">Operational Monitoring Parameter</p>
      </div>

      <form onSubmit={handleSubmit} className="space-y-8">
        {/* Basic Info */}
        <Section title="Informasi Dasar" icon={<Info size={18}/>}>
          <div className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Nama Pemeriksa</label>
              <input 
                type="text" required
                value={formData.inspectorName} onChange={e => setFormData({...formData, inspectorName: e.target.value})}
        ...
        {/* Electrical Section */}
        <Section title="Parameter Elektrik (Fan & Heater)" icon={<Zap size={18}/>}>
          <div className="space-y-6">
            {/* Fan Sub-section */}
            <div className="p-4 bg-slate-50 rounded-2xl space-y-4">
              <div className="flex items-center justify-between border-b border-slate-200/60 pb-3">
                <span className="text-xs font-black text-slate-500 uppercase tracking-widest">Fan Motor</span>
                <Toggle active={formData.fan_on} onClick={() => setFormData({...formData, fan_on: !formData.fan_on})} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <Input label="Fan Speed (%)" value={formData.fan_speed} onChange={v => setFormData({...formData, fan_speed: v})} />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Load Amperage (R / S / T)</label>
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Voltage (RS / ST / RT)</label>
                <div className="grid grid-cols-3 gap-2">
            ...
            <div className="p-4 border border-rose-100 bg-rose-50/30 rounded-2xl space-y-4">
              <div className="flex items-center justify-between border-b border-rose-100 pb-3">
                <span className="text-xs font-black text-rose-500 uppercase tracking-widest">Electric Heater</span>
                <Toggle active={formData.heater_on} onClick={() => setFormData({...formData, heater_on: !formData.heater_on})} />
              </div>
              {formData.heater_on && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="space-y-4 overflow-hidden">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-rose-400 uppercase tracking-widest">Amperage (R / S / T)</label>
                    <div className="grid grid-cols-3 gap-2">
        ...
        {/* Mechanical Section */}
        <Section title="Mechanical & Air" icon={<Wind size={18}/>}>
          <div className="space-y-6">
            <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Control Valve opening</label>
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-6">
               <DoubleInput label="Supply Air" valT={formData.supply_temp} valR={formData.supply_rh} setT={(v: string) => setFormData({...formData, supply_temp: v})} setR={(v: string) => setFormData({...formData, supply_rh: v})} />
               <DoubleInput label="Return Air" valT={formData.return_temp} valR={formData.return_rh} setT={(v: string) => setFormData({...formData, return_temp: v})} setR={(v: string) => setFormData({...formData, return_rh: v})} />
               <DoubleInput label="Fresh Air" valT={formData.fresh_temp} valR={formData.fresh_rh} setT={(v: string) => setFormData({...formData, fresh_temp: v})} setR={(v: string) => setFormData({...formData, fresh_rh: v})} />
               <Input label="Static Pressure (Pa)" value={formData.static_pressure} onChange={(v: string) => setFormData({...formData, static_pressure: v})} />
            </div>
          </div>
        </Section>

        {/* Filter Section */}
        <Section title="Media Filter Status" icon={<Filter size={18}/>}>
           <div className="space-y-4">
              <FilterToggle label="Pre-Filter" value={formData.filter_pre} onChange={v => setFormData({...formData, filter_pre: v})} />
              <FilterToggle label="Medium Filter" value={formData.filter_med} onChange={v => setFormData({...formData, filter_med: v})} />
              <FilterToggle label="HEPA Filter" value={formData.filter_hepa} onChange={v => setFormData({...formData, filter_hepa: v})} />
           </div>
        </Section>

        {/* Room & Spots Section */}
        <Section title="Kondisi Ruangan & Spot" icon={<LayoutList size={18}/>}>
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
               <Input label="Room Temp (°C)" value={formData.room_temp} onChange={v => setFormData({...formData, room_temp: v})} />
               <Input label="Diff. Pressure (Pa)" value={formData.room_diff_press} onChange={v => setFormData({...formData, room_diff_press: v})} />
            </div>
            
            <div className="space-y-3">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Temperature Mapping (5 Spot)</label>
            ...
            <div className="grid grid-cols-2 gap-4">
               <StatusSelect label="Vibration Motor" value={formData.vibration_status} onChange={(v: string) => setFormData({...formData, vibration_status: v})} options={["OK", "Abnormal"]} />
               <StatusSelect label="Condensate Drain" value={formData.drainage_status} onChange={(v: string) => setFormData({...formData, drainage_status: v})} options={["OK", "Abnormal"]} />
            </div>
          </div>
        </Section>
        ...
        <div className="space-y-2">
          ...
          <textarea 
            ...
            placeholder="Mohon isi jika ada temuan khusus..."
        ...
        <div className="fixed bottom-6 left-0 right-0 px-6 flex justify-center z-50">
          ...
          >
            ...
            <span>Simpan Log Operasional</span>
          </button>
        </div>
      </form>
