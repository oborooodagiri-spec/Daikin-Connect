            ...
            >
              {filteredResources.map((res, i) => (
                <motion.div
                  ...
                  className={`group relative bg-white border border-[#e6e9ef] rounded-[2rem] p-4 hover:border-[#0073ea]/30 hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-500 ${res.id === 'internal-rate-card' ? 'ring-2 ring-blue-500/10' : ''}`}
                >
                  <div className="relative aspect-[4/3] rounded-[1.5rem] overflow-hidden mb-6 bg-slate-50 border border-slate-100">
                    ...
                    <div className="absolute top-4 left-4 flex flex-wrap gap-2 pr-4">
                       <span className={`px-3 py-1.5 rounded-full backdrop-blur-md border border-slate-100 text-[9px] font-black uppercase tracking-widest shadow-sm ${res.id === 'internal-rate-card' ? 'bg-[#0073ea] text-white' : 'bg-white/90 text-[#0073ea]'}`}>
                          {res.category}
                       </span>
                  ...
                  <div className="px-2 pb-2 space-y-4">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2 mb-1">
                         <span className="text-[9px] font-black text-[#0073ea] bg-blue-50 px-2 py-0.5 rounded-md border border-blue-100">{res.projects?.name || "OFFICIAL"}</span>
                      </div>
                      ...
                      <div className="flex items-center gap-4 text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                         <div className="flex items-center gap-1.5"><Clock className="w-3 h-3" /> <RelativeTime date={res.created_at} /></div>
