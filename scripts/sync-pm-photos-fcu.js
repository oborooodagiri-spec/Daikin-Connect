/**
 * SYNC PREVENTIVE MAINTENANCE PHOTOS (FCU)
 * 
 * Extracts photos from the Plaza Indonesia cumulative report and links them 
 * to Preventive Maintenance records in the database.
 */
  ...
  for (let i = 0; i < sheetData.length; i++) {
    ...
    if (colB && colB !== 'UNIT') {
      if (colB.match(/^(FCU|AHU)\s/i)) {
        ...
        if (unitMatch) unitCodeNormalized = `${unitMatch[1].toUpperCase()} ${unitMatch[2].replace(/^0+/, '') || unitMatch[2]}`;
      } else {
        currentTenant = colB.replace(/^Tenant\s*/i, '').trim();
      }
    }
    lookup[i] = { day: currentDay, unit: currentUnit, unitCode: unitCodeNormalized, tenant: currentTenant, temuan: colE, tindakan: colF };
  }
  return lookup;
}
...
const normalize = (name) => {
  return (name || '').toUpperCase()
    .replace(/^(TENANT|AREA)\s+/i, '') // Remove leading TENANT or AREA
    .replace(/[^A-Z0-9\s]/g, '') // Remove punctuation like ' or .
    .replace(/\s*0+\d+$/, '') // Remove " 001" suffix
    .trim();
};

const getDayDate = (dayStr) => {
  const m = dayStr.match(/DAY\s*(\d+)/i);
  if (!m) return null;
...
async function main() {
  console.log('=== PREVENTIVE FCU PHOTO SYNC (V2.1) ===');
  ...
  const pmRecords = await prisma.service_activities.findMany({
    where: { type: 'Preventive', units: { project_ref_id: PROJECT_ID }, deleted_at: null },
    include: { units: { select: { room_tenant: true, unit_type: true } } }
  });
  console.log(`Loaded ${pmRecords.length} PM records (Total)`);

  const pmIndex = {};
  pmRecords.forEach(r => {
    const norm = normalize(r.units?.room_tenant);
    if (!pmIndex[norm]) pmIndex[norm] = [];
    pmIndex[norm].push(r);
  });
  ...
  for (const imgPos of allImagePositions) {
    ...
    const normTenant = normalize(tenant);
    let matchingPMs = pmIndex[normTenant];

    if (!matchingPMs || matchingPMs.length === 0) { skipped++; continue; }

    // Filter by FCU if that's what the user wants first
    // For now, let's process ALL and see the distribution
    const targetPMs = matchingPMs.filter(pm => pm.units.unit_type === 'FCU');
    const ahuPMs = matchingPMs.filter(pm => pm.units.unit_type === 'AHU');
    
    // User specifically said "mulai dari FCU"
    const pmsToUse = targetPMs.length > 0 ? targetPMs : (ahuPMs.length > 0 ? ahuPMs : []);
    if (pmsToUse.length === 0) { skipped++; continue; }

    try {
      const targetPM = pmsToUse[ (tenantRoundRobin[normTenant] || 0) % pmsToUse.length ];
      tenantRoundRobin[normTenant] = (tenantRoundRobin[normTenant] || 0) + 1;
      ...
      const fileName = `pm_${targetPM.units.unit_type.toLowerCase()}_${targetPM.id}_${Date.now()}_${extracted}.jpg`;
      ...
      await prisma.activity_photos.create({
        data: {
          ...
          caption: `${context.day || ''} - ${tenant} (${targetPM.units.unit_type})`.trim(),
          media_type: 'image'
        }
      });
      extracted++; linked++;
