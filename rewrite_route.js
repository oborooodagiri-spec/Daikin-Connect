...
// I will overwrite the entire file to avoid regex issues.
...
async function handleIncomingMessage(from: string, text: string) {
  ...
  // 1. If it's a known interactive ID payload or a top-level text command
...
async function sendProjectListMenu(from: string) {
  ...
  const sections = [{ title: "Your Projects", rows: rows.slice(0, 10) }]; // Meta limit is 10 rows
